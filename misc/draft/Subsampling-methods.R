...
subsample <- function(data, startTime, endTime, interval=c("24 hours","48 hours"),accuracy=c("3 mins","1 mins"),minIndiv=3,maxIndiv=4,mustIndiv=NULL, completeSetsOnly=TRUE, index.type){# here you set completeSetsOnly=TRUE by default and you leave a choice to set this parameter, ...
    ...
    if (index.type=="mci"){# ... but here you kind of make the choice of completeSetsOnly irrelevant, if index.type="mci". 
        completeSets = TRUE# You could also erase this if-else clause and just tell the user in the help, that completeSetsOnly must be TRUE, if index.type is "mci".
    }
    else{
